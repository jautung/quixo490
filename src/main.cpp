...
int main(int argc, char* argv[]) {
  try {
    ...
    if (prog == "play") {
      ...
      std::cout << "--------------------------------------------------------------------------------\n";
      if (playerXType != "opt") std::cout << "Player X init compute time (s): " << std::chrono::duration_cast<std::chrono::milliseconds>(gamePlayHandler->initTimeX).count()/1000.0 << " (" << std::chrono::duration_cast<std::chrono::milliseconds>(gamePlayHandler->initTimeX).count()/1000.0/numGames << " average per game)\n";
      if (playerOType != "opt") std::cout << "Player O init compute time (s): " << std::chrono::duration_cast<std::chrono::milliseconds>(gamePlayHandler->initTimeO).count()/1000.0 << " (" << std::chrono::duration_cast<std::chrono::milliseconds>(gamePlayHandler->initTimeO).count()/1000.0/numGames << " average per game)\n";
