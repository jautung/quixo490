...
class OptComputer {
  ...
  private:
    ...
    void initialScanClass(nbit_t numX, nbit_t numO, std::vector<result4_t> &results, sindex_t numStates);
    void valueIterateClass(nbit_t numX, nbit_t numO, std::vector<result4_t> &results, sindex_t numStates, std::vector<result4_t> &resultsOther, std::vector<result4_t> &resultsCachePlus, bool &updateMade);
